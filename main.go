...
func loadActivities(filename string) (map[string]Activity, error) {
	...
	return activitiesData.Activities, nil
}

// Generate a 365-day grid going backwards from current date
func generateGrid(activities map[string]Activity) [][]ContributionGrid {
	now := time.Now()
...
func initialModel(activities map[string]Activity) Model {
...
func (m Model) renderActivityGrid(activity Activity, activityKey string, activityNumber int) string {
	var s strings.Builder
...
func (m Model) getCellChar(cell ContributionGrid, activity Activity, activityKey string) string {
...
func (m Model) getCellColor(cell ContributionGrid, activity Activity, activityKey string) string {
	dateStr := cell.Date.Format("2006-01-02")
	for _, activityDate := range activity.Dates {
		if activityDate == dateStr {
...
func main() {
	...
	activities, err := loadActivities("data/activities.json")
	if err != nil {
	...
	if len(activities) == 0 {
		fmt.Println("ERROR: No activities found. Please check your data/activities.json file.")
		os.Exit(1)
	}

	// Create the model
	m := initialModel(activities)
